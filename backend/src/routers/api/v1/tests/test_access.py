...
# more like a get test, but takes a list of resources
@pytest.mark.anyio
@pytest.mark.parametrize(
    "mocked_provide_http_token_payload",
    [
...
async def test_user_get_access_policies_for_resources(
    async_client: AsyncClient,
    app_override_provide_http_token_payload: FastAPI,
    add_many_test_access_policies,
    current_user_from_azure_token,
    ...
    test_policy = await add_one_test_access_policy(test_policy)
    ...
    response = await async_client.post(
        "/api/v1/access/policy/resources",
        json=[str(resource_id_for_query), str(resource_id1)],
    )
    payload = response.json()

    assert response.status_code == 200

    target_policies.append(many_test_policies[0])
    target_policies.append(many_test_policies[5])
    target_policies.append(many_test_policies[9])
    target_policies.append(test_policy)
...
async def test_user_get_access_policies_for_identity(
    ...
    resource_id_third = str(uuid.uuid4())
    current_user = await current_user_from_azure_token(
        mocked_provide_http_token_payload
    )
    ...
    other_test_access_policy_for_current_user = {
        "resource_id": resource_id_other,
        "identity_id": current_user.user_id,
        "action": Action.own,
    }
    third_test_access_policy_for_current_user = {
        "resource_id": resource_id_third,
        "identity_id": current_user.user_id,
        "action": Action.own,
    }
...
async def test_user_get_access_policies_for_another_users_identity_being_owner_of_common_resources(
    ...
    third_test_access_policy_for_queried_user = {
        "resource_id": resource_id_third,
        "identity_id": identity_id_user2,
        "action": Action.own,
    }
...
async def test_user_get_access_policies_for_another_users_identity_without_owner_of_common_resources(
    ...
    # Access policies for the querying user - which is not owner of resource:
    write_test_access_policy_for_current_user = {
        "resource_id": resource_id,
        "identity_id": current_user.user_id,
        "action": Action.write,
...
async def test_admin_deletes_access_policy(
    ...
    assert len(read_payload) == 10
    # 1 deleted, but 1 created, when accessing the endpoint
    ...
    assert AccessPolicyRead(**read_payload[3]) == AccessPolicyRead(
        **policies_in_database[4].model_dump()
    )
    ...
    assert AccessPolicyRead(**read_payload[8]) == AccessPolicyRead(
...
async def test_user_deletes_access_policy(
    ...
    permission_for_test = await add_one_test_access_policy(
        {
            "resource_id": many_test_policies[1]["resource_id"],
            "identity_id": str(current_user.user_id),
            "action": Action.own,
        }
    )
    ...
    read_response = await async_client.get(
        f"/api/v1/access/policy/resource/{resource_id2}"
    )
    read_payload = read_response.json()

    assert len(read_payload) == 2
    # user has only access to 3 and deleted 1
...
async def test_user_tries_to_add_write_policy_where_an_owner_policy_exists_already(
    async_client: AsyncClient,
    app_override_provide_http_token_payload: FastAPI,
    add_many_test_access_policies,
    current_user_from_azure_token,
    ...
    await add_one_test_access_policy(write_test_access_policy_for_current_user)
    ...
    try:
        ...
        await add_one_test_access_policy(own_test_access_policy_for_current_user)
    except Exception as err:
...
async def test_user_get_access_permission_for_resources(
    async_client: AsyncClient,
    app_override_provide_http_token_payload: FastAPI,
    register_many_resources,
    add_many_test_access_policies,
    ...
    target_policies = [
        ...
        {
            "resource_id": str(resource_id5),
            "identity_id": str(current_user.user_id),
            "action": Action.own,
        },
        {
            "resource_id": str(resource_id7),
            "identity_id": str(current_user.user_id),
            "action": Action.own,
        },
    ]
...
async def test_get_creation_datetime_for_resources(
    ...
    assert response.status_code == 200
    assert database_logs[0].time.isoformat() == payload[0]
    assert database_logs[1].time.isoformat() == payload[1]
    assert database_logs[4].time.isoformat() == payload[2]
...
async def test_get_modification_datetime_for_resource(
    async_client: AsyncClient,
    app_override_provide_http_token_payload: FastAPI,
    add_many_test_access_logs,
    current_user_from_azure_token,
    ...
    assert response.status_code == 200
    assert payload == (database_logs[12].time).isoformat()
...
async def test_get_modification_datetime_is_creation_time_for_never_accessed_resource(
    async_client: AsyncClient,
    app_override_provide_http_token_payload: FastAPI,
    ...
    response_modified = await async_client.get(
        f"/api/v1/access/log/{demo_resources[2].id}/last-modified"
    )
    payload_modified = response_modified.json()

    response_created = await async_client.get(
        f"/api/v1/access/log/{demo_resources[2].id}/created"
    )
    payload_created = response_created.json()

    assert response_modified.status_code == 200
    assert response_created.status_code == 200
    assert payload_modified == payload_created
...
async def test_get_modification_datetime_for_resource_with_only_write_permission(
    async_client: AsyncClient,
    app_override_provide_http_token_payload: FastAPI,
    add_many_test_access_logs,
    current_user_from_azure_token,
...
async def test_get_modification_datetime_for_resource_with_only_read_permission(
    async_client: AsyncClient,
    app_override_provide_http_token_payload: FastAPI,
    add_many_test_access_logs,
    current_user_from_azure_token,
...
async def test_get_modification_datetime_for_resource_without_access_fails(
    async_client: AsyncClient,
    app_override_provide_http_token_payload: FastAPI,
    add_many_test_access_logs,
):
...
async def test_get_modification_datetime_for_resources(
    async_client: AsyncClient,
    app_override_provide_http_token_payload: FastAPI,
    add_many_test_access_logs,
    current_user_from_azure_token,
    ...
    assert response.status_code == 200
    assert database_logs[12].time.isoformat() == payload[0]
    assert database_logs[13].time.isoformat() == payload[1]
    assert database_logs[15].time.isoformat() == payload[2]
...
async def test_get_modification_datetime_for_resources_with_write_permission_on_one_resource_only(
    async_client: AsyncClient,
    app_override_provide_http_token_payload: FastAPI,
    add_many_test_access_logs,
    current_user_from_azure_token,
...
async def test_get_modification_datetime_for_resources_with_read_permission_on_resources(
    async_client: AsyncClient,
    app_override_provide_http_token_payload: FastAPI,
    add_many_test_access_logs,
    current_user_from_azure_token,
...
async def test_get_modification_datetime_for_resources_without_access_on_one_resource_fails(
    async_client: AsyncClient,
    app_override_provide_http_token_payload: FastAPI,
    current_user_from_azure_token,
    mocked_provide_http_token_payload,
...
async def test_get_last_access_log_for_resource(
    ...
    assert int(returned.id)
    assert returned.resource_id == database_logs[15].resource_id
    assert returned.identity_id == database_logs[15].identity_id
    assert returned.action == database_logs[15].action
    assert returned.status_code == database_logs[15].status_code
    assert returned.time == database_logs[15].time
