...
class BaseSQLModel(SQLModel):
    """Common base for all application table models with a UUID primary key.

    Declares `id` as `Optional[uuid.UUID]` because the UUID is generated by
    `default_factory=uuid.uuid4` only when the model is instantiated/validated,
    not when the schema is first defined. Subclasses (or the `create_model`
    factory) must redeclare `id` with the concrete `Field(...)` so the column
    gets the foreign key and primary key configuration. After
    `model_validate()`, callers can `assert obj.id is not None` to narrow the
    type.
    """
...
class GeneratedSQLModel(BaseSQLModel):
    ...
    The factory attaches the generated `Create`, `Read`, `Update`, and
    `Extended` schemas as class attributes on the returned table model.
    Declaring them here as `ClassVar[Type[SQLModel]]` lets type checkers
...
def create_model(
    ...
    if table:
        ...
        return cast("Type[GeneratedSQLModel]", TableModel)
    else:
        # Return a container class with nested schemas for model validation without a table.
        # The container is not actually a `BaseSQLModel` subclass; callers using
        # `table=False` access it only via its dynamically-attached schemas.
        Container = type(name, (BaseModel,), {})
        Container.Create = Create  # type: ignore[attr-defined]
        Container.Read = Read  # type: ignore[attr-defined]
        Container.Update = Update  # type: ignore[attr-defined]
        Container.Extended = Extended  # type: ignore[attr-defined]

        return cast("Type[GeneratedSQLModel]", Container)
