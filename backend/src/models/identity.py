...
class UserCreate(SQLModel):
    """Administrator-created user, including an optional provider identity."""

    # The internal id is always generated by the backend. Keep ignoring it here
    # for compatibility with existing REST and Socket.IO create clients.
    model_config = ConfigDict(extra="ignore")  # type: ignore[assignment]
    azure_user_id: Optional[uuid.UUID] = None
    azure_tenant_id: Optional[uuid.UUID] = None
...
class User(UserCreate, BaseSQLModel, table=True):
    ...
    linkedin_user_id: Optional[str] = Field(
        default=None,
        sa_column=Column(String(collation="C"), nullable=True, index=True, unique=True),
    )
...
class UserUpdate(SQLModel):
    ...
    model_config = ConfigDict(extra="ignore")  # type: ignore[assignment]
    # Provider identifiers may be echoed by legacy clients, but are selectors,
    # not mutable fields. Excluding them prevents reassignment during update.
    azure_user_id: Optional[uuid.UUID] = Field(default=None, exclude=True)
    azure_tenant_id: Optional[uuid.UUID] = Field(default=None, exclude=True)
    linkedin_user_id: Optional[str] = Field(default=None, exclude=True)
    # Socket.IO includes its resource selector in the payload; never update it.
    # It is deliberately untyped because legacy clients may send any selector;
    # the route path/event dispatch selects the actual database record.
    id: object | None = Field(default=None, exclude=True)
...
class UserAccountUpdate(SQLModel):
    """Editable account settings, without identity/link fields."""
...
class UserProfileUpdate(SQLModel):
    """Editable profile settings, without identity/link fields."""

    model_config = ConfigDict(extra="forbid")  # type: ignore[assignment]
    theme_color: (
        Annotated[str, StringConstraints(pattern=r"^#[0-9a-fA-F]{6}$")] | None
    ) = None
    theme_variant: Optional[ThemeVariants] = None
    contrast: Optional[float] = Field(default=None, ge=-1.0, le=1.0)


class MeUpdate(SQLModel):
    """Own-user update; provider identifiers and token claims are read-only."""

    model_config = ConfigDict(extra="forbid")  # type: ignore[assignment]
    is_active: Optional[bool] = None
    id: uuid.UUID
    user_account: Optional[UserAccountUpdate] = None
