...
- Application authentication and authorization policy is centralized in `backend/src/core/security.py`. Provider-token validation lives in `core/authentication/azure.py` and `linkedin.py`; `authentication/base.py` shares issuer dispatch and public signing-key caching. Security loads cached MSAL tokens by session id and provides guard/dependency helpers for HTTP and Socket.IO. See [`docs/architecture/security/README.md`](../docs/architecture/security/README.md#authentication-modules) for the module boundary.
...
- Generate and manage schema migrations through the existing `scripts/dev/postgres_migration_*.sh` and `scripts/stage_prod/postgres_migration_*.sh` wrappers. Follow the [PostgreSQL migration workflow](../docs/postgres/README.md#schema-migration-workflow), including each tree's generated template format and revision history.
- Do not add a dedicated test for every migration. For ordinary autogenerated schema changes, rely on the existing migration workflow and the required application tests described below. Add migration-specific tests only when custom data transformations or a concrete migration risk are not covered by those tests; explain that reason.
...
- Test all authentication, authorization, and access-control functionality, including guards and access policies. Use the existing `test_security.py` as a reference for authentication and authorization coverage.
- Test every externally exposed backend interface, including each REST endpoint and Socket.IO event. Cover successful requests and relevant failures, especially unauthenticated, unauthorized, and access-denied cases. Verify behavior through the exposed interface rather than relying only on lower-layer unit tests.
