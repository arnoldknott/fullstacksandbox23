...
<div class="w-50">
    <form id="post-message" class="flex flex-col">
        <md-filled-text-field label="Message" type="input" name="message" oninput={e => new_message = e.target.value} class="w-50"> </md-filled-text-field>
        <div class="ml-auto py-4">
            <md-filled-button type="submit" role="button" onclick={socket?.send(new_message)} onkeydown={(e) => e.key === 'Enter' && socket?.send(new_message)} tabindex="0">OK</md-filled-button>
        </div>
    </form>
</div>
