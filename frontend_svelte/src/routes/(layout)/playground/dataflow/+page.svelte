<script lang="ts">
	...
	const cleanedData = Object.fromEntries(
		Object.entries(data).filter(([key]) => key.startsWith('layoutServerTs') || key.startsWith('layoutTs') || key.startsWith('pageServerTs') || key.startsWith('pageTs'))
	);
	...
	const submitFunctionWithCallBack: SubmitFunction = async ({
		formElement,
		formData,
		action,
		cancel,
	...
	const submitFunctionWithCallBackAndApplyAction: SubmitFunction = async ({
		formElement,
		formData,
		action,
		cancel,
	...
	// enhanced submit function with callback and applyAction:
	const submitFunctionWithCallBackAndUpdate: SubmitFunction = async ({
		formElement,
		formData,
		action,
		cancel,
	...
	const submitFunctionWhereExpectedErrorCausesFail: SubmitFunction = async ({
		formElement,
		formData,
		action,
		cancel,
...
<div class="text-center">
	...
	<p class="p-4">
		Client side navigation to <button class="btn ml-10" onclick={redirect}>redirect page</button>
	</p>
</div>
...
<form class="m-2 rounded p-2 outline" method="post" action="?/named">
...
<form class="m-2 rounded p-2 outline" method="post" action="?/enhanced" use:enhance>
...
<form
	class="m-2 rounded p-2 outline"
	method="post"
	action="?/submitFunctionWithCallback"
	use:enhance={submitFunctionWithCallBack}
>
	<p class="title">Use:enhance</p>
	<p class="title-small {subTitleHeight}">
		Submit Function with Callback
		{#if success}
	...
	<div class="flex flex-row items-end gap-2">
		<div class="input-filled input-neutral grow">
			<label class="input-filled-label" for="inputSubmitWithCallback"
				>Submit Function with Callback</label
			>
			<input
				class="input input-sm"
				placeholder="send data to server"
				type="text"
				id="inputSubmitWithCallback"
				name="inputSubmitWithCallback"
			/>
		</div>
		<button class="btn btn-neutral">Send</button>
	</div>
...
<form
	class="m-2 rounded p-2 outline"
	method="post"
	action="?/submitFunctionWithCallBackAndApplyAction"
	use:enhance={submitFunctionWithCallBackAndApplyAction}
>
	<p class="title">Use:enhance</p>
	<p class="title-small {subTitleHeight}">Submit Function with Callback and applyAction()</p>
	<div class="flex flex-row items-end gap-2">
		<div class="input-filled input-info grow">
			<label class="input-filled-label" for="inputSubmitWithCallBackAndApplyAction"
				>SubmitFct, callback, applyAction</label
			>
			<input
				class="input input-sm"
				placeholder="send data to server"
				type="text"
				id="inputSubmitWithCallBackAndApplyAction"
				name="inputSubmitWithCallBackAndApplyAction"
			/>
		</div>
...
<form
	class="m-2 rounded p-2 outline"
	method="post"
	action="?/submitFunctionWithCallBackAndUpdate"
	use:enhance={submitFunctionWithCallBackAndUpdate}
>
	<p class="title">Use:enhance</p>
	<p class="title-small {subTitleHeight}">Submit Function with Callback and update()</p>
	<div class="flex flex-row items-end gap-2">
		<div class="input-filled input-success grow">
			<label class="input-filled-label" for="inputSubmitFunctionWithCallBackAndUpdateubmitUpdate"
				>SubmitFct, callback, update</label
			>
			<input
				class="input input-sm"
				placeholder="send data to server"
				type="text"
				id="inputSubmitFunctionWithCallBackAndUpdate"
				name="inputSubmitFunctionWithCallBackAndUpdate"
			/>
		</div>
...
<form class="m-2 rounded p-2 outline" method="post" action="?/redirectServerSide" use:enhance>
	...
	<div class="flex flex-row items-end gap-2">
		<div class="input-filled input-error grow">
			<label class="input-filled-label" for="inputRedirectServerSide"
				>Trigger redirect() on server</label
			>
			<input
				class="input input-sm"
				placeholder="send data to server"
				type="text"
				id="inputRedirectServerSide"
				name="inputRedirectServerSide"
			/>
		</div>
...
<form
	class="m-2 rounded p-2 outline"
	method="post"
	action="?/expectedErrorCausesFail"
	use:enhance={submitFunctionWhereExpectedErrorCausesFail}
>
	<p class="title">Expected error in action</p>
	<p class="title-small {subTitleHeight}">
		Returns fail() from action
		{#if fail}
	...
	<div class="flex flex-row items-end gap-2">
		<div class="input-filled input-warning grow">
			<label class="input-filled-label" for="inputExpectedErrorCausesFail"
				>Trigger fail() on server</label
			>
			<input
				class="input input-sm"
				placeholder="send data to server"
				type="text"
				id="inputExpectedErrorCausesFail"
				name="inputExpectedErrorCausesFail"
			/>
		</div>
...
<form class="m-2 rounded p-2 outline" method="post" action="?/unexpectedErrorInBackend" use:enhance>
	<p class="title">Unexpected error in backend</p>
	<p class="title-small {subTitleHeight}">
		Throwing Error() in action and handle redirect through Svelte error() to <code
			>+error.svelte</code
		>
	</p>
	<div class="flex flex-row items-end gap-2">
		<div class="input-filled input-error grow">
			<label class="input-filled-label" for="inputUnexpectedErrorInBackend"
				>Trigger error() on server</label
			>
			<input
				class="input input-sm"
				placeholder="send data to server"
				type="text"
				id="inputUnexpectedErrorInBackend"
				name="inputUnexpectedErrorInBackend"
			/>
		</div>
		<button class="btn btn-error">Send</button>
	</div>
