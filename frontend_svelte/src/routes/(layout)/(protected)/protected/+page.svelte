...
<div class="mb-5 grid grid-cols-1 gap-8 md:grid-cols-3">
	{#each resources as resource (resource.name)}
		<NavigationCard title={resource.name} href={resource.url}>{resource.description}</NavigationCard
		>
	{/each}
</div>
...
<div class="mb-5 grid grid-cols-1 gap-8 md:grid-cols-3">
	{#each identities as identity (identity.name)}
		<NavigationCard title={identity.name} href={identity.url}>{identity.description}</NavigationCard
		>
	{/each}
</div>
...
<div class="mb-5 grid grid-cols-1 gap-8 md:grid-cols-3">
	{#each services as service (service.name)}
		<NavigationCard title={service.name} href={service.url}>{service.description}</NavigationCard>
	{/each}
</div>
...
<div class="mb-5 grid grid-cols-1 gap-8 md:grid-cols-3">
	{#each application as app (app.name)}
		<NavigationCard title={app.name} href={app.url}>{app.description}</NavigationCard>
	{/each}
</div>
