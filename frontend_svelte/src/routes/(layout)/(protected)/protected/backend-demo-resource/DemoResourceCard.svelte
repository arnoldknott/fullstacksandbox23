<script lang="ts">
	...
	// let { id, name, description, language, category, category_id, tags }: { id: string, name: string; description: string, language: string; category?: string, category_id?: string, tags: string[] } = $props();
    let flag = $state(language === 'en-US' ? 'united-states' : 
                        language === "da-DK" ? 'denmark' :
                        language === "de-DE" ? 'germany' : false
    );
</script>
...
{#snippet header()}
    <div class="flex justify-between">
        ...
        <div>
            {#if category}
                <span id={category_id} class="text-label-small md:text-label lg:text-label-large badge badge-secondary shadow-sm shadow-secondary">
                    {category}
                </span>
            {/if}
            {#if flag}
