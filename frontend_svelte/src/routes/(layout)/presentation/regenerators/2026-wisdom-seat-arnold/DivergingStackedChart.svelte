			...
			>
				...
				<div class="flex items-center justify-start gap-x-10">
					{#each legendRight as category (category)}
						{#if category && colorClasses[category]}
							<div class="flex items-center gap-3">
								<span class={`h-6 w-6 rounded ${colorClasses[category] ?? 'bg-base-content'}`}
								></span>
								{category}
							</div>
						{/if}
					...
					{#each aggregatedByYear as entry (entry.year)}
						...
						<div class="grid h-24 grid-cols-[14rem_1fr] items-center gap-6">
							...
							<div class="relative h-16">
								{#if rightFarCategory && colorClasses[rightFarCategory]}
									<div
								...
								{#if rightNearCategory && colorClasses[rightNearCategory]}
									<div
