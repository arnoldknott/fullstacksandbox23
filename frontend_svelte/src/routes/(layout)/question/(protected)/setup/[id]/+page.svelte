<script lang="ts">
	...
	let hideNewNumericalAnswerCard = $state(true);
	let numericalViewMode = $state<'grid' | 'list' | 'table'>('table');
</script>
...
{#if numericalSocketio?.pendingEntities[0]}
	<Card
		id="add-numericals"
		title="Add Numerical Answers"
		closeButton
		extraClasses="label-large"
		bind:hidden={hideNewNumericalAnswerCard}
	>
		<!-- <p class="label">
		{@render warning()} Add questions with tabs for new and existing, where the existing questions get
		the button to copy and link. Also a copy all selected and link all selected.
		...
		{#snippet footer()}
			<div class="ml-5 flex flex-row justify-end gap-4">
				<button
					...
					aria-label="Cancel"
					onclick={() => {
						hideNewNumericalAnswerCard = true;
					}}><span class="icon-[tabler--x] size-5"></span>Cancel</button
				>
				<!-- <button
					class="btn btn-primary-container btn-gradient shadow-outline rounded-full shadow"
					aria-label="Save new presentation"
					onclick={() => {
						numericalSocketio.submitBulk(parentId, inheritPending);
						hideNewNumericalAnswerCard = true;
						// For buld submit the new pending needs manual creation:
						numericalSocketio.createPending();
					}}><span class="icon-[tabler--send-2] size-5"></span>
					Save
				</button> -->
			</div>
...
{:else if !numericalSocketio?.pendingEntities[0] && !hideNewNumericalAnswerCard}
	<div class="label text-error" transition:slide={{ duration: 600 }}>
		<span class="icon-[svg-spinners--12-dots-scale-rotate] size-6"></span>connecting ...
	</div>
{/if}

<Card id="linked-numerical" title="Linked numerical answers" extraClasses="label-large">
	{#snippet header()}
		<div class="flex justify-between">
			<Title id="existingQuestions" class="grow">Overview</Title>
			<div class="flex flex-row items-center justify-center">
				...
				<div class="join shadow-outline rounded-full shadow-sm">
					<button
						aria-label="Grid"
						class="btn join-item btn-secondary btn-gradient btn-sm shadow-outline rounded-l-full py-4 shadow {numericalViewMode !==
						'grid'
							? 'opacity-60'
							: ''}"
						onclick={() => (numericalViewMode = 'grid')}
					>
						<span class="icon-[gridicons--grid] size-5"></span>
					</button>
					<button
						aria-label="List"
						class="btn join-item btn-secondary btn-gradient btn-sm shadow-outline py-4 shadow {numericalViewMode !==
						'list'
							? 'opacity-60'
							: ''}"
						onclick={() => (numericalViewMode = 'list')}
					>
						<span class="icon-[material-symbols--view-list-outline] size-5"></span>
					</button>
					<button
						aria-label="Table"
						class="btn join-item btn-secondary btn-gradient btn-sm shadow-outline rounded-r-full py-4 shadow {numericalViewMode !==
						'table'
							? 'opacity-60'
							: ''}"
						onclick={() => (numericalViewMode = 'table')}
					>
						<span class="icon-[material-symbols--table-outline] size-5"></span>
					</button>
				</div>
	...
	{/snippet}
	<div class="w-full overflow-x-auto {numericalViewMode !== 'grid' ? 'hidden' : ''}">
		<p class="bg-warning text-warning-content rounded-lg p-4">
			Grid view mode is not developed yet
		</p>
	</div>
	<div class="w-full overflow-x-auto {numericalViewMode !== 'list' ? 'hidden' : ''}">
		<p class="bg-warning text-warning-content rounded-lg p-4">
			List view mode is not developed yet
		</p>
	</div>
	<div class="w-full overflow-x-auto {numericalViewMode !== 'table' ? 'hidden' : ''}">
		<!-- TBD: move ActionButtons to components -->
		{#snippet numericalActionsCell(numerical: NumericalExtended)}
			<ActionButtons
				resourceId={numerical.id}
				accessRight={numericalSocketio?.accessRights[numerical.id]}
				socketio={numericalSocketio}
			/>
		{/snippet}
		<Table
			columns={[
				{
					header: text('Value'),
					cell: field<NumericalExtended>('value'),
					headerClass: 'w-3/5',
					cellClass: 'max-w-0'
				},
				{
					header: text('Tolerance'),
					cell: field<NumericalExtended>('tolerance'),
					headerClass: 'text-center',
					cellClass: 'text-center'
				},
				{
				...
				{
					header: text('Actions'),
					cell: snippet(numericalActionsCell),
					headerClass: 'w-px text-center whitespace-nowrap',
					cellClass: 'w-px py-1 text-center align-middle whitespace-nowrap'
				}
