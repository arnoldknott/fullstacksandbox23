...
<div class="grid grid-cols-3 gap-4">
    <div class="w-48 ">
        ...
        <input class="input" type="color" id="colorPicker" name="color-picker" bind:value={values.sourceColor} />
        
    </div>
    <div class="relative w-48">
        <label class="label label-text" for="themeVariant">Variant</label>
        <select class="select select-floating max-w-sm" aria-label="Select variant" id="themeVariant" bind:value={values.variant}>
    ...
    <div class="w-48">
        ...
        <input type="range" min={contrastMin} max={contrastMax} step={contrastStep} class="range w-full"  aria-label="contrast" id="contrast" bind:value={values.contrast}/>
