...
async def test_user_moves_child_order_down_insert_after(
    ...
    # Move the order of a protected child forward:
    response_moving = await async_client.post(
        f"/api/v1/protected/resource/{str(parent_resource_id)}/move/{str(mocked_protected_children[1].id)}/after/{str(mocked_protected_children[4].id)}"
    )
    payload_moving = response_moving.json()
...
async def test_user_moves_child_order_up_insert_after(
    ...
    # Move the order of a protected child forward:
    response_moving = await async_client.post(
        f"/api/v1/protected/resource/{str(parent_resource_id)}/move/{str(mocked_protected_children[5].id)}/after/{str(mocked_protected_children[2].id)}"
    )
    payload_moving = response_moving.json()
...
async def test_get_protected_child_resource_from_a_parent_missing_inheritance(
    ...
    # Make a POST request to create the protected child as a child of a protected resource
    response = await async_client.post(
        f"/api/v1/protected/resource/{created_protected_resource.id}/child?inherit=False",
        json=many_test_protected_child_resources[0],
    )
...
async def test_get_protected_grand_child_resource_through_inheritance_via_child_from_parent(
    ...
    response_grandchild = await async_client.post(
        f"/api/v1/protected/child/{created_protected_child.id}/grandchild?inherit=True",
        json=many_test_protected_grandchild_resources[0],
    )
    assert response.status_code == 201
